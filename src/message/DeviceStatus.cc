...
int32_t DeviceStatus::serialize(void* buf, uint32_t bufsize) const {
  ...
  int32_t wRstVspVersion;
  assert(vspVersion);
  wRstVspVersion = caps->write(vspVersion->c_str());
  if (wRstVspVersion != CAPS_SUCCESS) return wRstVspVersion;
  int32_t wRstTurenVersion;
  assert(turenVersion);
  wRstTurenVersion = caps->write(turenVersion->c_str());
  if (wRstTurenVersion != CAPS_SUCCESS) return wRstTurenVersion;
...
int32_t DeviceStatus::deserialize(void* buf, uint32_t bufSize) {
  ...
  int32_t rRstVspVersion = caps->read_string(*vspVersion);
  if (rRstVspVersion != CAPS_SUCCESS) return rRstVspVersion;
  if (!turenVersion) turenVersion = std::make_shared<std::string>();
  int32_t rRstTurenVersion = caps->read_string(*turenVersion);
  if (rRstTurenVersion != CAPS_SUCCESS) return rRstTurenVersion;
